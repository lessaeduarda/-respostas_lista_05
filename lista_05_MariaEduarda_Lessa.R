...
# O número máximo de meses apresentando crescimento para que a empresa receba o incentivo fiscal seria de 4 meses e meio, nos dados analisados, 
# no entanto, os números de meses são discretos, o que significa que as empresas que apresentam até 4 meses de crescimento receberão o benefício.
...
#### Parece haver uma relação entre o tamanho da empresa e o número de meses com crescimento, já que as médias para as empresas P, M e G foram,
#### respectivamente, 5.5, 5.86 e 6. Nas empresas pequenas houve menor variância entre os meses de crescimento, enquanto nas empresas grandes, maior.
...
# Já foi visto na letra b) que não há valor abaixo de 8.830744 na base, por isso é necessário apenas criar filtro para eliminar valores acima de 25.56926.

# Calcular média da nova base:
mean(cid_invest_novo$investimento)

#### A média é uma medida sensível a valores extremos, por isso, com a eliminação do investimento da cidade A, de 26, a nova média obtida é menor
#### do que a original.
...
#### Em média, para o estímulo "B" o tempo de reação é menor. Os valores de reação para o estímulo "A" variaram mais em relação à média e também
#### estão mais dispersos que os valores de "B", como é possível notar a partir do desvio padrão.
...
#### No estímulo "A" os tempos de reação foram maiores e seus valores estãoo mais dispersos do que no estímulo "B". Não existe assimetria acentuada 
#### em "A" ou "B", visto que a linha da mediana está próxima do meio da caixa nos dois casos (ainda assim, "A" apresenta maior assimetria do que "B"). 
#### Para o estímulo "A" foram observados 2 resultados extremos (45 e 55), enquanto para "B", 3 (25, 26 e 35).
...
# Calcular média e desvio padrão de renda (em salários mínimos ["renda_sm"])  e percentual gasto com saúde ("saude_perc"):
summary(renda_sm)
sd(renda_sm)
...
#### As variáveis apresentam forte correlação negativa, ou seja, é possível observar que quanto maior a renda da família, menor o gasto percentual 
#### dessa renda com saúde.
...
# Utilizar função "sample.size.prop" para descobrir número necessário de eleitores consultados para erro de 0.05: 

sample.size.prop(0.05, P = 0.5, N = Inf, level = 0.95)

## letra b)

# Utilizar função "sample.size.prop" para descobrir número necessário de eleitores consultados para erro de 0.02: 
sample.size.prop(0.02, P = 0.5, N = Inf, level = 0.95)

#### Comparando resultados de a) e b): Quanto maior o tamanho da amostra, mais próximas suas medidas serão dos parâmetros populacionais, 
#### por isso, quanto menor o erro, maior o tamanho da amostra. 

## letra c)

# Utilizar função "sample.size.prop" para descobrir número necessário de eleitores consultados para erro de 0.02, quando já sabemos o posicionamento
# de 25% dos indivíduos da amostra:
...
#### Erro do tipo 1: A hipótese nula é rejeitada quando é, de fato, verdadeira. Nesse caso, um erro do tipo 1 seria cometido se, de fato, não houvesse 
#### relação entre a ideologia do partido e o posicionamento dos seus integrantes quanto a descriminalização das drogas e fosse assumido no estudo que 
#### há uma relação.

#### Erro do tipo 2: A hipótese nula NÃO é rejeitada, quando, na verdade, deveria ser. No exemplo da questão o erro do tipo 2 seria assumir que a ideologia
#### do partido não tem relação com o posicionamento de seus representantes quanto a descriminalização das drogas, quando, na verdade, esta relação existe.
...
#### Ao analisar o valor do qui-quadrado (242,42) e compará-lo com o seu valor crítico (3,841), levando em conta os graus de liberdade 
#### (neste caso igual a 1), é possível concluir que existe forte associação entre as variáveis, com o p valor < 0,05.
...
#### É possível observar que a análise da relação entre a variação do PIB e o percentual de votos recebidos pelo candidato do partido incumbente,
#### entre os anos de 1876 e 2008, permitiu rejeitar a hipótese nula, apresentando maior coeficiente de correlação e menor p-valor do que a análise 
